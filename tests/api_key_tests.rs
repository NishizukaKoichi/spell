...
async fn test_api_key_generation_format() {
    ...
    assert!(full_key.starts_with("sk_live_"), "Key should start with sk_live_");
    assert_eq!(full_key.len(), "sk_live_".len() + 32,
               "Key should be prefix + 32 random characters");
    assert!(full_key["sk_live_".len()..].chars().all(|c| c.is_ascii_alphanumeric()),
...
async fn test_api_key_test_prefix() {
    ...
    assert!(full_key.starts_with("sk_test_"), "Test key should start with sk_test_");
...
async fn test_argon2_hashing() {
    ...
    let parsed_hash = PasswordHash::new(&hash)
        .expect("Hash should be valid PHC format");

    // Verify the hash matches the original key
    let argon2 = Argon2::default();
    assert!(argon2.verify_password(full_key.as_bytes(), &parsed_hash).is_ok(),
...
async fn test_argon2_wrong_key_fails() {
    ...
    let argon2 = Argon2::default();
    assert!(argon2.verify_password(wrong_key.as_bytes(), &parsed_hash).is_err(),
...
async fn test_create_api_key_endpoint_returns_key_and_hash() {
    ...
    let app = test::init_service(
        App::new()
            .route("/v1/keys", web::post().to(mock_create_api_key))
    ).await;
    ...
    assert!(api_key.starts_with("sk_live_") || api_key.starts_with("sk_test_"),
